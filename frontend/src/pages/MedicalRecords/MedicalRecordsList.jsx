...
function MedicalRecordsList() {
  const { patientId } = useParams();
    ...
    try {
      ...
      const recordsResponse = await axios.get(`http://localhost:5000/api/medical-records/patients/${patientId}`);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4">
      <div className="max-w-6xl mx-auto">
        ...
        <button className="bg-green-700 text-white px-4 py-2 rounded hover:bg-green-900">Add new record</button>
        </Link>
              ...
              <tbody>
                {records.map((record)=>{
                  ...
                  <td className="p-3 text-left">{record.department}</td>
                    ...
                    <td className="p-3 text-left">{record.diagnoses || 'N/A'}</td>
                     <td className="p-3 text-left">
                      <Link

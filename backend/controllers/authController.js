...
export const register = async (req, res) => {
    try {
        await Promise.all([
            ...
            // ✅ Doctor-specific validations (only if role = doctor)
            body('role')
                .optional()
                .isIn(['admin', 'customer', 'doctor']).withMessage('Invalid role').run(req),

            body('specialization')
                .if(body('role').equals('doctor'))
                .notEmpty().withMessage('Specialization is required for doctors').run(req),

            body('qualification')
                .if(body('role').equals('doctor'))
                .notEmpty().withMessage('Qualification is required for doctors').run(req),
            ...
            body('availableDays')
                .if(body('role').equals('doctor'))
                .isArray().withMessage('Available days must be an array').run(req),

            body('availableTime')
                .if(body('role').equals('doctor'))
                .notEmpty().withMessage('Available time is required').run(req)
        ]);
        ...
        const {
            ...
            role = 'customer', specialization, qualification, experience, availableDays, availableTime
        } = req.body;
